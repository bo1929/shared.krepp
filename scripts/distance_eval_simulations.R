...
df <- vroom("./../results/all_simulations-results.tsv")
...
sample_n(df, 10000) %>% filter(!is.na(DIST_EST) & !ahigh) %>%
  ggplot() +
  aes(x=DIST_TRUE, y=DIST_EST) +
  geom_point(alpha = 0.033) +
...
sample_n(df, 1000000) %>% filter(!is.na(DIST_EST) & !ahigh) %>%
  ...
  theme_half_open() +
  background_grid() +
  labs(x="True HD bin", y="Estimated HD per read") +
  theme(axis.text.x = element_text(angle = 45, hjust = 0.99))
...
sample_n(df, 1000000) %>% filter(!ahigh) %>%
  ...
  theme_half_open() +
  background_grid() +
  labs(x="True HD bin", y="Portion mapped") +
  scale_y_continuous(labels = percent) +
  theme(axis.text.x = element_text(angle = 45, hjust = 0.99))
