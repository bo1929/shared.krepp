...
dfr %>% 
  ggplot(aes(mash_dist,mean_dist, group=interaction(ref_id,query_id,method))) +
...
dfr %>%
  ...
  geom_point(aes(color="bowtie", y=mean_dist_bowtie), alpha=0.95, size=2, shape=16) +
  geom_point(aes(color="krepp", y=mean_dist_krepp), alpha=0.95, size=2, shape=17) +
  geom_segment(alpha=0.3, linewidth = 0.15) +
  geom_abline() + theme_minimal_grid() +
  scale_color_brewer(palette = "Set1", direction = -1) +
...
ggsave2("../figures/meanHD_versus_MashD-pairs_in_both.pdf", width = 5.5, height = 5)
...
dfs <- vroom(
  "../results/read_summary-WoLv2-alignment_comparison-1M.csv",
